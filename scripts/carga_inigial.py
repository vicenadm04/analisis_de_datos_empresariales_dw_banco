...
try:
    ...
    for row in resultados_clientes:
        if row[3] is None:
            row = ("X",) + row[1:]  # Reemplaza "valor_por_defecto" con el valor deseado
        cursor.execute("INSERT INTO dw_banco.dim_prestatario (codigo_prestatario, fecha_nacimiento, cantidad_hijos, ingreso_mensual, direccion, ciudad, codigo_provincia, provincia, fecha_inicio_vigencia, genero, propiedad) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (row[0], row[4], 0, row[15]/12, row[7], row[8], row[9], row[10], row[14], row[3], row[18]))
